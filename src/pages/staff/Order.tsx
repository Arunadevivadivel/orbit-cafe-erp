...
const StaffOrder = () => {
  ...
  // Map some names for image lookup
  const getImage = (name: string) => {
    const map: Record<string, string> = { "Fresh Orange": "dish-orange-juice", "Sandwich": "dish-sandwich", "Brownie": "dish-brownie", "Americano": "dish-americano" };
    return dishImages[name] || undefined;
  };
            ...
            {categories.map((cat) => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-300 ${
                  activeCategory === cat ? "gradient-primary text-primary-foreground shadow-glow" : "bg-card border border-border text-muted-foreground hover:text-foreground"
                }`}>{cat}</button>
            ))}
          </div>
            ...
            {filtered.map((item) => {
              ...
              return (
                <motion.button key={item.id} whileTap={{ scale: 0.97 }} onClick={() => addToCart(item)}
                  className={`glass-card overflow-hidden text-left hover:shadow-glow transition-all duration-300 relative ${inCart ? "ring-2 ring-primary/40" : ""}`}>
                  <div className="w-full h-24 bg-secondary/30">
                    {img ? (
                  ...
                  {inCart && (
                    <span className="absolute top-2 right-2 w-6 h-6 gradient-primary rounded-full text-xs font-bold text-primary-foreground flex items-center justify-center shadow-glow">
                      {inCart.qty}
                    </span>
                  )}
                </motion.button>
        ...
        <div className="w-80 glass-card p-5 flex flex-col shrink-0">
              ...
              {cart.map((item) => {
                ...
                return (
                  <motion.div key={item.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/30">
                    {img && <img src={img} alt={item.name} className="w-10 h-10 rounded-lg object-cover shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{item.name}</p>
                      <p className="text-xs text-muted-foreground">₹{item.price} each</p>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <button onClick={(e) => { e.stopPropagation(); updateQty(item.id, -1); }} className="w-7 h-7 rounded-full bg-card border border-border flex items-center justify-center hover:bg-secondary"><Minus className="w-3 h-3" /></button>
                      <span className="text-sm font-semibold w-6 text-center">{item.qty}</span>
                      <button onClick={(e) => { e.stopPropagation(); updateQty(item.id, 1); }} className="w-7 h-7 rounded-full gradient-primary text-primary-foreground flex items-center justify-center"><Plus className="w-3 h-3" /></button>
                    </div>
                    <p className="text-sm font-bold text-foreground w-14 text-right">₹{item.price * item.qty}</p>
                    <button onClick={(e) => { e.stopPropagation(); removeItem(item.id); }} className="text-muted-foreground hover:text-destructive"><Trash2 className="w-3.5 h-3.5" /></button>
                  </motion.div>
                );
              })}
            </AnimatePresence>
          ...
          {cart.length > 0 && (
            <div className="border-t border-border pt-4 mt-4 space-y-2">
              ...
              <div className="flex justify-between text-sm text-muted-foreground"><span>GST (5%)</span><span>₹{gst.toLocaleString()}</span></div>
              <div className="flex justify-between text-base font-bold text-foreground pt-2 border-t border-border"><span>Total</span><span>₹{total.toLocaleString()}</span></div>
              <Button variant="hero" size="lg" className="w-full mt-3" onClick={() => setShowPayment(true)}>Proceed to Payment</Button>
            </div>
          )}
        </div>
      </div>
        ...
        {showPayment && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-foreground/20 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="glass-card p-8 w-full max-w-md relative">
              <button onClick={() => setShowPayment(false)} className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center hover:bg-secondary"><X className="w-4 h-4" /></button>
              ...
              <div className="space-y-3 mb-6">
                {[{ label: "Cash", icon: Banknote }, { label: "Card", icon: CreditCard }, { label: "UPI", icon: Smartphone }].map((method) => (
              ...
              <Button variant="hero" size="lg" className="w-full" onClick={() => { setShowPayment(false); setCart([]); }}>Complete Order</Button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
